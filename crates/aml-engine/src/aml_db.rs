...
pub struct AccountProfileDb {
    pub address: [u8; 20],
    pub sent_amounts: VecDeque<(u64, SerializableU256, [u8; 20])>,
    pub recv_amounts: VecDeque<(u64, SerializableU256, [u8; 20])>,
    pub total_sent: SerializableU256,
    pub total_received: SerializableU256,

    // Mule-specific fields
    pub unique_senders: Vec<([u8; 20], u64)>,    // sender -> last_seen_block
    pub unique_recipients: Vec<([u8; 20], u64)>, // recipient -> last_seen_block
...
impl From<&AccountProfile> for AccountProfileDb {
    fn from(profile: &AccountProfile) -> Self {
        Self {
            ...
            sent_amounts: profile.sent_amounts.iter()
                .map(|(b, amt, recip)| (*b, (*amt).into(), *recip.0.as_ref()))
                .collect(),

            recv_amounts: profile.recv_amounts.iter()
                .map(|(b, amt, sender)| (*b, (*amt).into(), *sender.0.as_ref()))
                .collect(),

            total_sent: profile.total_sent.into(),
            total_received: profile.total_received.into(),

            unique_senders: profile.unique_senders.iter()
                .map(|(addr, &last_seen)| (*addr.0.as_ref(), last_seen))
                .collect(),

            unique_recipients: profile.unique_recipients.iter()
                .map(|(addr, &last_seen)| (*addr.0.as_ref(), last_seen))
                .collect(),
...
impl From<AccountProfileDb> for AccountProfile {
    fn from(db: AccountProfileDb) -> Self {
        Self {
            ...
            sent_amounts: db.sent_amounts.into_iter()
                .map(|(b, amt, recip)| (b, amt.into(), Address(FixedBytes::from(recip))))
                .collect(),

            recv_amounts: db.recv_amounts.into_iter()
                .map(|(b, amt, sender)| (b, amt.into(), Address(FixedBytes::from(sender))))
                .collect(),

            total_sent: db.total_sent.into(),
            total_received: db.total_received.into(),

            unique_senders: db.unique_senders.into_iter()
                .map(|(addr, last_seen)| (Address(FixedBytes::from(addr)), last_seen))
                .collect(),

            unique_recipients: db.unique_recipients.into_iter()
                .map(|(addr, last_seen)| (Address(FixedBytes::from(addr)), last_seen))
                .collect(),

            first_seen_block: db.first_seen_block,
        }
    }
}
